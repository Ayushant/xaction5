        ...
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="premium-card animate-fadeIn">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-4xl font-black text-white mb-2">MISSION CONTROL</h2>
                <p className="text-white/80 text-xl">Select and execute strategic missions</p>
              </div>
              <div className="text-right">
                <p className="text-sm text-white/70 uppercase tracking-wider font-bold mb-1">Missions Completed</p>
                <p className="text-5xl font-black text-transparent bg-gradient-to-br from-green-400 to-cyan-500 bg-clip-text">
                  {myScores.length}
                </p>
              </div>
            </div>
          ...
          {/* Tabs */}
          <div className="premium-card animate-scaleUp" style={{ animationDelay: '0.1s' }}>
            <nav className="flex -mb-4">
              ...
              >
                ...
                {activeTab === 'available' && (
                  <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-cyan-400 to-purple-600 neon-glow"></div>
                )}
              </button>
              <button
              ...
              >
                ...
                {activeTab === 'completed' && (
                  <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-green-400 to-cyan-600 neon-glow"></div>
                )}
              </button>
            </nav>
          </div>
          ...
          <div className="animate-slideInBottom" style={{ animationDelay: '0.2s' }}>
            {loading ? (
              <div className="flex flex-col justify-center items-center py-20">
                <div className="gaming-spinner mb-6"></div>
                <span className="text-white/80 text-2xl font-bold">Loading missions...</span>
              </div>
            ) : activeTab === 'available' ? (
              // Available Missions
                ...
                {quizzes.filter(q => !isQuizCompleted(q._id)).length === 0 ? (
                  <div className="col-span-full">
                    <div className="premium-card text-center py-20">
                      ...
                      <p className="text-white/80 text-xl mb-2">No available missions</p>
                      <p className="text-white/60">Check back later for new strategic challenges</p>
                    </div>
                  </div>
                ) : (
                  quizzes
                    ...
                    .map((quiz, index) => (
                      <div
                        key={quiz._id}
                        className="premium-card hover:scale-105 transition-all cursor-pointer group"
                        style={{ animationDelay: `${index * 0.1}s` }}
                      >
                        {/* Mission Badge */}
                        <div className="flex items-center gap-4 mb-6">
                          <div className="w-16 h-16 bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl flex items-center justify-center neon-glow group-hover:scale-110 transition-transform">
                            <svg className="w-8 h-8 text-white" fill="currentColor" viewBox="0 0 24 24">
                              <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
                            </svg>
                          </div>
                          <div className="flex-1">
                            <h3 className="text-2xl font-black text-white group-hover:text-cyan-300 transition-colors line-clamp-2">
                              {quiz.title}
                            </h3>
                          </div>
                        </div>
                        ...
                        <div className="space-y-3 mb-6">
                          <div className="flex items-center justify-between text-sm glass-panel p-3 rounded-lg">
                            ...
                            <span className="font-black text-white">{quiz.questions?.length || 0}</span>
                          </div>
                          <div className="flex items-center justify-between text-sm glass-panel p-3 rounded-lg">
                            <span className="text-white/70 flex items-center gap-2">
                            ...
                            <span className="font-black text-green-400">Strategic</span>
                          </div>
                          <div className="flex items-center justify-between text-sm glass-panel p-3 rounded-lg">
                            <span className="text-white/70 flex items-center gap-2">
                            ...
                            <span className={`px-3 py-1 rounded-full text-xs font-black ${
                              quiz.difficulty === 'Easy' ? 'bg-green-500/20 text-green-300 border border-green-400/30' :
                              quiz.difficulty === 'Hard' ? 'bg-red-500/20 text-red-300 border border-red-400/30' :
                        ...
                        <button
                          ...
                          className="w-full btn-gaming py-4 text-lg group-hover:scale-105 transition-transform"
                        >
              ...
              <div className="space-y-6">
                {myScores.length === 0 ? (
                  <div className="premium-card text-center py-20">
                    ...
                    <p className="text-white/80 text-xl mb-4">No completed missions yet</p>
                    <button
                      onClick={() => setActiveTab('available')}
                      className="btn-gaming px-8 py-3 text-lg inline-block"
                    >
                  ...
                  myScores.map((score, index) => {
                    ...
                    return (
                      <div
                        key={score._id}
                        className="premium-card hover:scale-[1.02] transition-all animate-fadeIn"
                        style={{ animationDelay: `${index * 0.1}s` }}
                      >
                        <div className="flex flex-col md:flex-row gap-6">
                          {/* Score Badge */}
                          <div className="flex-shrink-0">
                            <div className="w-32 h-32 bg-gradient-to-br from-green-500 to-cyan-600 rounded-2xl flex flex-col items-center justify-center neon-glow">
                              <div className="text-4xl font-black text-white mb-1">
                                {Math.round(score.totalScore)}%
                              </div>
                              <div className="text-white/80 text-sm font-bold uppercase">Score</div>
                            </div>
                          </div>
                          ...
                          <div className="flex-1">
                            <div className="flex items-start justify-between mb-4">
                              <div>
                                ...
                                <span className={`px-4 py-2 rounded-full text-sm font-black border-2 ${
                                  score.totalScore >= 80 ? 'bg-green-500/20 text-green-300 border-green-400/40 neon-glow' :
                                  score.totalScore >= 60 ? 'bg-blue-500/20 text-blue-300 border-blue-400/40' :
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                              <div className="glass-panel p-4 rounded-lg">
                                <p className="text-white/60 text-xs uppercase tracking-wider font-bold mb-1">Challenges</p>
                              ...
                              {score.accuracy && (
                                <div className="glass-panel p-4 rounded-lg">
                                  <p className="text-white/60 text-xs uppercase tracking-wider font-bold mb-1">Accuracy</p>
                              ...
                              <div className="glass-panel p-4 rounded-lg">
                                <p className="text-white/60 text-xs uppercase tracking-wider font-bold mb-1">Date</p>
